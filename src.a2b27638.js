...
function selectHorizontally(view, dir, mods) {
  ...
  if (sel instanceof _prosemirrorState.TextSelection) {
    if (!sel.empty || mods.indexOf("s") > -1) {
      ...
      return false;
    } else if (!(result.mac && mods.indexOf("m"))) {
      var $head = sel.$head,
...
},{"prosemirror-transform":"node_modules/prosemirror-transform/dist/index.es.js","prosemirror-model":"node_modules/prosemirror-model/dist/index.es.js"}],"node_modules/prosemirror-inputrules/dist/index.es.js":[function(require,module,exports) {
"use strict";
...
},{"./bundle-url":"node_modules/parcel-bundler/src/builtins/bundle-url.js"}],"node_modules/prosemirror-view/style/prosemirror.css":[function(require,module,exports) {
...
var testDecoToDom = function testDecoToDom(decoClass, side) {
...
var pluginKey = new _prosemirrorState.PluginKey("side-test-plugin");
var noOfDecos = 3;
var decoStartPos = 6;
var MODIFY_OFFSET = "MODIFY_OFFSET";
var DECREMENT_OFFSET = "DECREMENT";
var INCREMENT_OFFSET = "INCREMENT";

var getSide = function getSide(cursorPos, offset, index) {
  return cursorPos === decoStartPos && offset > index ? -1 : 1;
};

var createPluginDecorations = function createPluginDecorations(state, decoClass, index) {
  var decos = new Array(noOfDecos).fill(undefined).map(function (_, index) {
    var _pluginKey$getState = pluginKey.getState(state),
        offset = _pluginKey$getState.offset;

    var orderOffset = (index + 1) / noOfDecos / 2;
    var _state$selection = state.selection,
        from = _state$selection.from,
        to = _state$selection.to;
    var isRange = from !== to;
    var side = isRange ? orderOffset : getSide(from, offset, index);
    var relativeSide = side + orderOffset;
    return _prosemirrorView.Decoration.widget(decoStartPos, testDecoToDom(decoClass, relativeSide), {
      key: Math.random(),
      side: relativeSide,
  ...
  document.getElementById(getDebugId(index)).innerHTML = "Widget 'side' properties, in render order: ".concat(decos.map(function (_) {
    return _.spec.side.toString().substring(0, 6);
  }).join(", "));
  return _prosemirrorView.DecorationSet.create(state.doc, decos);
};

var createPlugin = function createPlugin(decoClass, index) {
  return new _prosemirrorState.Plugin({
    key: pluginKey,
    props: {
      decorations: function decorations(state) {
        return createPluginDecorations(state, decoClass, index);
      }
    },
    state: {
      ...
      apply: function apply(tr, plState, oldState, newState) {
        var offset = plState.offset;
        var modifyOffset = tr.getMeta(MODIFY_OFFSET);
    ...
    appendTransaction: function appendTransaction(trs, oldState, newState) {
      var _pluginKey$getState2 = pluginKey.getState(newState),
          offset = _pluginKey$getState2.offset;

      var $oldCursor = oldState.selection.$cursor;
      var $cursor = newState.selection.$cursor;

      if (!$cursor || !$oldCursor) {
        return;
      }

      var movement = $cursor.pos - $oldCursor.pos;

      if ( // Haven't moved
      movement === 0 || // Just moved into deco position
      $oldCursor.pos !== decoStartPos) {
        return;
      }

      if (movement > 0 && offset < noOfDecos || movement < 0 && offset > 0) {
        var modifyOffset = movement > 0 ? INCREMENT_OFFSET : DECREMENT_OFFSET;
        var tr = newState.tr.setMeta(MODIFY_OFFSET, modifyOffset).setSelection(_prosemirrorState.Selection.near(oldState.selection.$cursor));
        return tr;
      }
...
var createStateFromContent = function createStateFromContent(content, decoClass, index) {
  var plugin = createPlugin(decoClass, index);
  return _prosemirrorState.EditorState.create({
    doc: _prosemirrorModel.DOMParser.fromSchema(schema).parse(content),
    plugins: [].concat(_toConsumableArray((0, _prosemirrorExampleSetup.exampleSetup)({
      schema: schema,
      menuBar: false
    })), [plugin])
  });
...
var getDebugId = function getDebugId(decoClass) {
  return "debug-".concat(decoClass);
};

var examples = [["Example with decorations only, inline", "<p>An example document</p>", "test-deco-inline"], ["Example with decorations only, inline-block", "<p>An example document</p>", "test-deco-inline-block"], ["Example with decorations and mark, inline", "<p><test-mark>An ex</test-mark>ample document</p>", "test-deco-inline"], ["Example with decorations and mark, inline-block", "<p><test-mark>An ex</test-mark>ample document</p>", "test-deco-inline-block"]];
var root = document.getElementById("root");
examples.forEach(function (_ref, index) {
  var _ref2 = _slicedToArray(_ref, 3),
      description = _ref2[0],
      content = _ref2[1],
      decoClass = _ref2[2];

  var docRootEl = document.createElement("div");
  ...
  var state = createStateFromContent(contentEl, decoClass, index);
  new _prosemirrorView.EditorView(docRootEl, {
    state: state
  });
});
},{"prosemirror-model":"node_modules/prosemirror-model/dist/index.es.js","prosemirror-state":"node_modules/prosemirror-state/dist/index.es.js","prosemirror-view":"node_modules/prosemirror-view/dist/index.es.js","prosemirror-example-setup":"node_modules/prosemirror-example-setup/dist/index.es.js","prosemirror-view/style/prosemirror.css":"node_modules/prosemirror-view/style/prosemirror.css","prosemirror-example-setup/style/style.css":"node_modules/prosemirror-example-setup/style/style.css","./styles.css":"src/styles.css"}],"node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
